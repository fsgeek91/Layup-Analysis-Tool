...
if OUTPUT_STRENGTH == true
    ...
    if (noFailStress == true) && (noFailStrain == true)
        %{
            Since the strength calculation has been requested, at least one
            of FAIL_STRESS or FAIL_STRAIN must be defined for the ply!
        %}
        fprintf(['[ABD ERROR] The strength calculation requies at leas',...
            't FAIL_STRESS or FAIL_STRAIN material properties\n']);
        return
    end
else
    noFailStress = true;
...
if (OUTPUT_STRENGTH == true) && (printTensor == 1.0)
    [MSTRS, TSAIH, TSAIW, AZZIT, MSTRN, XT, XC, YT, YC, S, C, B,...
        E11, E22, G12, V12, XET, XEC, YET, YEC, SE] =...
        ...
        abd.internal_strength.main(noFailStress, noFailStrain, XT, XC,...
        YT, YC, S, C, B, E11, E22, G12, V12, XET, XEC, YET, YEC, SE,...
        S_ply_aligned, nPlies, nPlies_points, SECTION_POINTS);
    ...
    if OUTPUT_OPTIMISED{1.0} == true
        ...
        BEST_SEQUENCE =...
            ...
            deltaT, deltaM, Nxx, Nyy, Nxy, Mxx, Myy, Mxy, E11, E22, V12,...
            G12);
    end
else
    % Initialise values to default
...
abd.internal_outputToFile(dateString, outputLocation, OUTPUT_STRENGTH,...
    ...
    TSAIW, AZZIT, MSTRN, noFailStress, noFailStrain, SECTION_POINTS,...
    OUTPUT_PLY_POINTS, plyBuffer, thickness,...
    OUTPUT_ENVELOPE, ENVELOPE_MODE, outputApproximate, BEST_SEQUENCE,...
    OUTPUT_OPTIMISED, OUTPUT_FIGURE)
