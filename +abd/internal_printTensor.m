function [] = internal_printTensor(fid, OUTPUT_ENVELOPE,...
    ...
    E_therm_xy, E_moist_xy, E_therm_aligned, E_moist_aligned, nPlies,...
    ...
    for i = 1.0:nPlies
        ...
        if isempty(currentOutputPoints) == true
            ...
            for sp = 1.0:length(currentOutputPoints)
                ...
                fprintf(fid, ['%-7.0f%-30s%-14g%-14g%-14g%-14g%-14g%-1',...
...
if OUTPUT_ENVELOPE == true
    ...
    fprintf(fid, '%-7s%-30s%-14g%-14g%-14g%-14g%-14g%-14g\n', 'ALL',...
        envelopeString, E_ply_xy_envelope_11, E_ply_xy_envelope_22,...
        E_ply_xy_envelope_12, E_ply_aligned_envelope_11,...
        ...
        if isempty(currentOutputPoints) == true
            ...
            for sp = 1.0:length(currentOutputPoints)
                ...
                fprintf(fid, ['%-7.0f%-30s%-14g%-14g%-14g%-14g%-14g%-1',...
                    ...
                    E_ply_aligned_i(1.0, sp), E_ply_aligned_i(2.0, sp),...
...
if isempty(E_therm_xy) == false &&...
        (any(any(E_therm_xy)) == true || any(any(E_therm_aligned)) == true)
    fprintf(fid, ['\nStress-free strain due to thermal process at user',...
        '-selected locations:\n']);
    fprintf(fid, ['PLY    SECTION POINT                 E_Therm_xx    ',...
        'E_Therm_yy    E_Therm_xy    E_Therm_11    E_Therm_22    E_The',...
        'rm_12    \n']);
    ...
    if OUTPUT_ENVELOPE == true
        ...
        fprintf(fid, '%-7s%-30s%-14g%-14g%-14g%-14g%-14g%-14g\n', 'ALL',...
            envelopeString, E_therm_xy_envelope_11,...
            E_therm_xy_envelope_22, E_therm_xy_envelope_12,...
            E_therm_aligned_envelope_11, E_therm_aligned_envelope_22,...
            E_therm_aligned_envelope_12);
    else
        for i = 1.0:nPlies
            %{
            ...
            if isempty(currentOutputPoints) == true
                ...
                E_therm_aligned_i = E_therm_aligned(:,...
                    currentOutputPoints);

                for sp = 1.0:length(currentOutputPoints)
                    thicknessFraction = sprintf(['%.0f (fraction = %.5',...
                        'g)'], currentOutputPoints(sp),...
                        thickness(currentOutputPoints(sp)));

                    fprintf(fid, ['%-7.0f%-30s%-14g%-14g%-14g%-14g%-14',...
                        'g%-14g\n'], i, thicknessFraction,...
                        E_therm_xy_i(1.0, sp), E_therm_xy_i(2.0, sp),...
                        E_therm_xy_i(3.0, sp),...
                        E_therm_aligned_i(1.0, sp),...
                        E_therm_aligned_i(2.0, sp),...
                        E_therm_aligned_i(3.0, sp));
                end
            end
...
if isempty(E_moist_xy) == false &&...
        (any(any(E_moist_xy)) == true || any(any(E_moist_aligned)) == true)
    fprintf(fid, ['\nStress-free strain due to moisture process at use',...
        'r-selected locations:\n']);
    fprintf(fid, ['PLY    SECTION POINT                 E_Moist_xx    ',...
        'E_Moist_yy    E_Moist_xy    E_Moist_11    E_Moist_22    E_Moi',...
        'st_12    \n']);
    ...
    if OUTPUT_ENVELOPE == true
        ...
        fprintf(fid, '%-7s%-30s%-14g%-14g%-14g%-14g%-14g%-14g\n',...
            'ALL', envelopeString, E_moist_xy_envelope_11,...
            E_moist_xy_envelope_22, E_moist_xy_envelope_12,...
            E_moist_aligned_envelope_11, E_moist_aligned_envelope_22,...
            E_moist_aligned_envelope_12);
    else
        for i = 1.0:nPlies
            %{
                Extract the strains for the current section point for the
            ...
            if isempty(currentOutputPoints) == true
                ...
                E_moist_aligned_i = E_moist_aligned(:,...
                    currentOutputPoints);
                ...
                for sp = 1.0:length(currentOutputPoints)
                    ...
                    fprintf(fid, ['%-7.0f%-30s%-14g%-14g%-14g%-14g%-14',...
                        'g%-14g\n'], i, thicknessFraction,...
                        E_moist_xy_i(1.0, sp), E_moist_xy_i(2.0, sp),...
                        E_moist_xy_i(3.0, sp),...
                        E_moist_aligned_i(1.0, sp),...
                        E_moist_aligned_i(2.0, sp),...
                        E_moist_aligned_i(3.0, sp));
                end
            end
