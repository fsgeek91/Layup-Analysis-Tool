...
%%
    ...
    methods(Static = true, Access = public)
        ...
        %% GET DATA FROM OUTPUT_OPTIMISED
        function [error, output] = getSettings(OUTPUT_OPTIMISED, noFailStress, noFailStrain, noHashin, noLaRC05, OUTPUT_STRENGTH, isUserFcn)
            % Initialise output
            error = false;
            output = cell(1.0, 4.0);
            ...
            elseif ischar(argument) == true
                if (strcmpi(argument, 'mstrs') == false) && (strcmpi(argument, 'tsaih') == false) && (strcmpi(argument, 'hoffman') == false) &&...
                        (strcmpi(argument, 'tsaiw') == false) && (strcmpi(argument, 'azzit') == false) && (strcmpi(argument, 'mstrn') == false) &&...
                        (strcmpi(argument, 'hashin') == false) && (strcmpi(argument, 'larc05') == false) && (strcmpi(argument, 'ucrt') == false)
                    ...
                    elseif (strcmpi(argument, 'larpfcrt') == true) || (strcmpi(argument, 'larmfcrt') == true) || (strcmpi(argument, 'larkfcrt') == true) ||...
                        ...
                        fprintf('[ERROR] OUTPUT_OPTIMISED(1) must be one of the following parameters: MSTRS, TSAIH, HOFFMAN, TSAIW, AZZIT, MSTRN, HASHIN, LARC05 or UCRT\n');
                    end
                ...
                elseif (noLaRC05 == true) && (OUTPUT_STRENGTH == true) && (strcmpi(argument, 'larc05') == true)
                    ...
                    error = true;
                    return
                elseif (isUserFcn == false) && (strcmpi(argument, 'ucrt') == true)
                    %{
                        The strength calculation is enabled without a
                        user-defined failure criterion, but UCRT is
                        requested as the optimiser failure/damage
                        initiation criterion
                    %}
                    fprintf('[ERROR] Requested UCRT criterion for optimisation, but a user-defined failure criterion has not been specified in OUTPUT_STRENGTH(1)\n');
            ...
            parfor i = 1.0:nPermutations                
                ...
                switch lower(failureCriterion)
                    case 'mstrs' % Maximum stress
                        CRITERION =...
                            ...
                            abd.internal_strength.getMstrs(S_ply_aligned, XT, XC, YT, YC, S);
                    case 'tsaih' % Tsai-Hill
                        CRITERION =...
                            ...
                            abd.internal_strength.getTsaih(parameter, S_ply_aligned, XT, XC, YT, YC, S);
                    case 'hoffman' % 2D Hoffman
                        CRITERION =...
                            ...
                            abd.internal_strength.getHoffman(parameter, S_ply_aligned, XT, XC, YT, YC, S, zeros(1.0, nPlies_points), linspace(-1.0, -1.0, nPlies_points));
                    case 'tsaiw' % Tsai-Wu
                        CRITERION =...
                            ...
                            abd.internal_strength.getTsaiw(parameter, S_ply_aligned, XT, XC, YT, YC, S, C12, B12);
                    case 'azzit' % Azzi-Tsai-Hill
                        CRITERION =...
                            ...
                            abd.internal_strength.getAzzit(parameter, nPlies_points, S_ply_aligned, XT, XC, YT, YC, S);
                    case 'mstrn' % Maximum strain
                        CRITERION =...
                            ...
                            abd.internal_strength.getMstrn(nPlies_points, S_ply_aligned, E11, E22, V12, G12, XET, XEC, YET, YEC, SE);
                    case 'hashin' % Hashin
                        [HSNFTCRT, HSNFCCRT, HSNMTCRT, HSNMCCRT] =...
                            ...
                            abd.internal_strength.getHashin(nPlies_points, S_ply_aligned, ALPHA, XHT, XHC, YHT, YHC, SHX, SHY);
                    ...
                    case 'ucrt' % User-defined failure criterion
                        ...
                        CRITERION = ...
                            fcnHandle(INFO, -1.0.*ones(1.0, nPlies_points), MATERIAL_MECH, MATERIAL_FAIL, struct('E_MIDSPAN', E_midspan, 'E_PLY_XY', E_ply_xy, 'S_PLY_XY',...
                            S_ply_xy, 'E_PLY_ALIGNED', E_ply_aligned, 'S_PLY_ALIGNED', S_ply_aligned, 'E_THERM_XY', E_therm_xy, 'E_HYDRO_XY', E_hydro_xy, 'E_THERM_ALIGNED',...
                            E_therm_aligned, 'E_HYDRO_ALIGNED', E_hydro_aligned)); %#ok<PFBNS>
                    otherwise
